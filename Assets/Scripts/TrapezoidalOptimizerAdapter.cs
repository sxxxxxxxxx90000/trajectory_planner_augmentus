...
namespace Trajectory_Planner_Augmentus
{
    public class TrapezoidalOptimizerAdapter : AbstractOptimizerAdapter
    {
        public override List<TrajectoryPoint> Plan(
            List<double[]> path, double dt, double vMax, double aMax, double dMax)
        {
            ...
            if (vMax <= 0 || aMax <= 0 || dMax <= 0) throw new ArgumentException("vMax, aMax, dMax must be > 0.");
            ...
            // Asymmetric trapezoid/triangle parameters
            double da = 0.5 * vMax * vMax / aMax; // distance to accelerate to vMax
            double dd = 0.5 * vMax * vMax / dMax; // distance to decelerate from vMax to 0
            bool triangular = (da + dd > L);

            double ta, tc, td, totalT, vPeak;

            if (triangular)
            {
                // Peak limited by distance with asymmetric a/d:
                // L = v^2/(2a) + v^2/(2d) => v = sqrt( 2 L a d / (a + d) )
                vPeak = Math.Sqrt(2.0 * L * aMax * dMax / (aMax + dMax));
                ta = vPeak / aMax;
                td = vPeak / dMax;
                tc = 0.0;
                totalT = ta + td;
            ...
            else
            {
                ...
                ta = vPeak / aMax;
                td = vPeak / dMax;
                double accelDist = 0.5 * aMax * ta * ta; // == da
                double decelDist = 0.5 * dMax * td * td; // == dd
                double cruiseDist = L - (accelDist + decelDist);
                tc = cruiseDist / vPeak;
                totalT = ta + tc + td;
            }
            ...
            for (int k = 0; k <= steps; k++)
            {
                ...
                if (!triangular)
                {
                    ...
                    {
                        // decelerate
                        double tdLoc = t - (ta + tc);
                        double sA = 0.5 * aMax * ta * ta; // da
                        double sC = vPeak * tc;
                        s = sA + sC + vPeak * tdLoc - 0.5 * dMax * tdLoc * tdLoc;
                        sdot = Math.Max(0.0, vPeak - dMax * tdLoc);
                    }
                }
                else
                {
                    if (t <= ta)
                    {
                        // accelerate (triangular)
                        s = 0.5 * aMax * t * t;
                        sdot = aMax * t;
                    }
                    else
                    {
                        // decelerate (triangular)
                        double tdLoc = t - ta;
                        double sAtPeak = 0.5 * aMax * ta * ta; // vPeak^2/(2a)
                        s = sAtPeak + vPeak * tdLoc - 0.5 * dMax * tdLoc * tdLoc;
                        sdot = Math.Max(0.0, vPeak - dMax * tdLoc);
                    }
                }
